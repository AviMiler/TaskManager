...
### דרך 2: כתוסף Chrome (Extension)

האפליקציה ארוזה כתוסף Manifest V3 ורצה ישירות מתוך Chrome / Edge:

1. פתח `chrome://extensions` (או `edge://extensions`).
2. הפעל **מצב מפתח / Developer mode** (מתג בפינה הימנית-עליונה).
3. לחץ **Load unpacked / טען לא ארוז** ובחר את תיקיית הפרויקט הזו (זו שמכילה את `manifest.json`).
4. לחץ על אייקון ה‑TaskBoard בסרגל הכלים — האפליקציה תיפתח בטאב משלה.

הנתונים נשמרים ב‑`localStorage` של התוסף (נשמרים בין הפעלות). תכונת **"קובץ משותף לצוות"** עובדת גם בתוסף, כי עמודי התוסף הם הקשר מאובטח (secure context).

> הערה למפתחים: עמודי תוסף MV3 חוסמים `inline scripts` ו‑`inline onclick`. לכן ה‑`onclick=""` הומרו ל‑`data-action=""` שמטופלים ע"י `actions.js`, וה‑`APP_CONFIG` עבר ל‑`config.js`. אין צורך בהרשאות מיוחדות.

### דרך 3: עם שרת מקומי (מומלץ, חובה לעבודת צוות)
...
- **בעלות על פרויקטים**: כל פרויקט חדש משויך אוטומטית למי שיצר אותו (כמו משימות). פרויקטים ישנים שנוצרו לפני העדכון נחשבים "משותפים" ומוצגים לכולם.
- **הצג רק שלי**: בתפריט בחירת הפרויקט יש מתג "הצג רק שלי". כשהוא דלוק רואים רק את הפרויקטים שיצרת, ורק משימות שיצרת או שמשויכות אליך (לפי שדה "אחראי"). המצב נשמר בדפדפן. זהו סינון תצוגה בלבד — לא הגנת אבטחה.
